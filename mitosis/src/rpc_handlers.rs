...
pub(crate) fn handle_fork_resume(input: &BytesMut, output: &mut BytesMut) -> usize {
    ...
    let (handler_id, auth_key): (usize, usize) = (fork_resume.handler_id, fork_resume.auth_key);
    crate::log::debug!(
        "[handle_swap_descriptor] start. handler id:{}, auth_key:{}",
        handler_id,
        auth_key
    );
    ...
    if let Some(meta) = dfs.get_descriptor_ref(handler_id) {
        ...
        return match dst.serialize(output) {
            ...
            false => {
                crate::log::error!(
                    "fail to serialize descriptor. handler id:{}, auth key:{}",
                    handler_id,
                    auth_key
                );
                0
