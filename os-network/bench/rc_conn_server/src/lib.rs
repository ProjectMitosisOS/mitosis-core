...
declare_global!(
    RCONTEXTS,
    alloc::boxed::Box<alloc::vec::Vec<KRdmaKit::device::RContext<'static>>>
);
...
impl linux_kernel_module::KernelModule for Module<'_> {
    fn init() -> linux_kernel_module::KernelResult<Self> {
        ...
        for i in 0..nic_count::read() {
            let ctx = driver.devices()[i as usize].open().unwrap();
            unsafe {
        ...
        for i in 0..nic_count::read() {
            ...
            let rctrl = RCtrl::create(service_id as u64, unsafe {
                &RCONTEXTS::get_ref()[i as usize]
            });
            rctrls.push(rctrl.unwrap());
        }

        Ok(Self { rctrls: rctrls })
    }
}
