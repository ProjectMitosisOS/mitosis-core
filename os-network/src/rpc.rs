...
/// A RPC caller is the combination of a receiver and several connected sessions.
///
...
where
    R: Receiver,
    SS: RPCConn,
    R: GetContext<Context = <<SS as RPCConn>::ReqPayload as AllocMsgBuf>::Context>,
    SS::ReqPayload: ToBytes,
{
    ...
    ///
    /// # Note: This call will crash if the session has already connected
    ///
    pub fn connect(
        &mut self,
        session_id: usize,
...
/// The trait to allocate message buffer. The corresponding context is generated by the trait `GetContext`
///
...
/// The trait to get a generic context. This trait should cooperate with other traits.
///
